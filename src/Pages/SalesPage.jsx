...
const SalesPage = ({ onMenuClick }) => {
   ...
   const validSales = filteredData.filter((sale) => sale.status !== 1);

// Calculate totals
const totalSales = validSales.length;
const totalAmount = validSales.reduce((sum, sale) => sum + Number(sale.totalAmount), 0);
                    ...
                    <Table
                        ...
                        onRow={(record) => {
                            if (record.status === 1) {
                                return {
                                    style: {
                                        backgroundColor: "#fff1f0", // Ant Design "error" light background
                                        color: "#ff4d4f",           // Ant Design red text
                                    },
                                };
                            }
                            return {};
